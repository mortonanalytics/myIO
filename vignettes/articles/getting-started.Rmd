...
myIO supports 36 chart types. See the [Chart Types](chart-types.html) vignette
for examples, or run `myio_list_chart_types()` for the complete list.
...
| Function | Purpose |
|----------|---------|
| `setBrush()` | Rectangle selection; returns selected rows as data |
| `setAnnotation()` | Click-to-label data points; export annotations as CSV |
| `setLinked()` | Crosstalk linked brushing across charts |
| `setSlider()` | Parameter sliders that trigger Shiny recomputation |
| `dragPoints()` | Make points draggable (with live regression refit) |
