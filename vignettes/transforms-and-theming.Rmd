...
The `ci` transform accepts `method` (`"lm"`, `"loess"`, or `"polynomial"`),
`level` (default 0.95), and `interval` (`"confidence"` or `"prediction"`).
Use `degree` for polynomial and LOESS fits and `span` for LOESS. Map `x_var`
and `y_var`; the transform supplies `low_y` and `high_y`.
