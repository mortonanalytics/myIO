...
test_that("setmargin deprecated alias still works", {
  expect_warning(
    widget <- myIO::setmargin(myIO::myIO(), top = 75),
    "deprecated"
  )
  expect_equal(widget$x$options$margin$top, 75)
})
...
test_that("setColorScheme deprecated colorShceme param still works", {
  expect_warning(
    widget <- myIO::setColorScheme(myIO::myIO(),
      colorShceme = list("green")
    ),
    "deprecated"
  )
  expect_equal(widget$x$options$colorScheme[[1]], list("green"))
})
...
test_that("all option functions return htmlwidget", {
  ...
  expect_s3_class(myIO::setColorScheme(w, colorScheme = list("red")), "htmlwidget")
