...
This runs weekly, on Saturday, so the queue is stocked before Monday's `backlog-pipeline`. Frequency is not a quota. A run that surfaces nothing genuinely new files nothing and says so, and that is a correct run, not a failed one — the ruled-out list in memory exists precisely so later runs can be short. Re-filing a known issue under a new number, or filing a candidate you padded to have something to show, costs more than the empty run it was meant to avoid.
