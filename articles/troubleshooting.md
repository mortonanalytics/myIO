...
**Error:** `setSlider(): \`param\` must be a single character string,
not numeric (123).\`

The function tells you exactly what it expected and what you gave it.
Check the argument types in
...
**Error:** `setLinked(): \`shared_data\` must be a SharedData object,
not data.frame.\`
...
object, not a raw data frame. Wrap your data first.
...
You passed something other than a
[`myIO()`](https://mortonanalytics.github.io/myIO/reference/myIO.md)
widget to a setter function. Make sure the first argument in your pipe
is [`myIO()`](https://mortonanalytics.github.io/myIO/reference/myIO.md).
...
validation may have removed them. These errors are not visible in R by
default.
...
1.  **Run
    ...
    for guidance on where to find the error
